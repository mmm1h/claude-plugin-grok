...
[![CI](https://img.shields.io/github/actions/workflow/status/mmm1h/claude-plugin-grok/pull-request-ci.yml?branch=main&label=CI)](https://github.com/mmm1h/claude-plugin-grok/actions/workflows/pull-request-ci.yml)
[![Version](https://img.shields.io/github/package-json/v/mmm1h/claude-plugin-grok)](https://github.com/mmm1h/claude-plugin-grok)
[![License](https://img.shields.io/github/license/mmm1h/claude-plugin-grok)](LICENSE)
[![Node](https://img.shields.io/badge/node-%3E%3D18.18-brightgreen)](package.json)

> Claude Code → local Grok CLI for read-only reviews, delegated tasks, transcript handoff, and tracked background jobs.

Not Grok→Codex; no Codex app-server.
...
### Background jobs
```text
1. Start  --background on review / rescue / transfer
2. Wait   /grok:status <id> --wait --with-result   # or /grok:result <id> --wait
3. Live   /grok:status <id> --logs
4. Stop   /grok:cancel
```

Optional: `/grok:result <id> --out bundle.json`, then companion `cleanup --dry-run` before prune.
...
**Defaults:**
- Headless timeout: **1h** without `--timeout-ms`
- Prompt cap: **16 MiB** UTF-8 (`--prompt-file`)
- Resume: only confirmed ended tasks in current Claude session + workspace
- `cancelled`: only after signal delivery or confirmed exit
- Transfer: lossy (source hash + omissions)
- State dir: `~/.claude/grok-companion/<repo>-<hash>/`

**Review evidence:**
- ≤2 files / ≤256 KiB tracked → inline
- Larger → `self-collect` via `read_file` / `grep` / `list_dir`
- Dirty branch scope prefers clean commit-range
- True oversize fails closed
